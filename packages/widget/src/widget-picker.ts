...
export function enterFeedbackMode(el: FeedbackWidgetEl): void {
  ...
  // Nothing to type into yet, at any width: the banner IS the prompt, and the
  // field opens on the element a click or tap picks. Round 3 rested the mode
  // in a focused composer about the whole page, and the owner's verdict on it
  ...
  const onMove = (ev: PointerEvent) => {
    ...
    // A composer means its element is chosen. Repainting the hover would take
    // the outline off the element the comment is being written about, which
 ...
 * margin at its element's height (`widget-card.ts` places it), headed with
 * what it is on. At phone width it is ONE row along the bottom (`.quick`) —
 * Cancel, the field, Post — the shape of the doc page's composer, which the
 * owner preferred (2026-09-11). No "on <element>" line there: the outline on
 * the element already says it, and the line and a Done button cost two rows
...
function showComposer(el: FeedbackWidgetEl, anchor: ElementAnchor, target: HTMLElement): void {
  ...
  // and the ones being left are kept on the element they were about, rather
  // than carried over the top of them.
  ...
  composer.innerHTML =
    (quick ? '' : `<div class="composer-snippet">${head}</div>`) +
  ...
  const post = async (): Promise<void> => {
    ...
    // Commenting is a MODE: posting leaves you in it, so several comments in
    // a row cost one entry and one exit — the next click or tap on the page
    // opens the next one.
  };
