...
export class LiveDocFanout {
  ...
  trackShareSocket(ws: ShareAuthorizedSocket): void {
    if (!ws.data?.shareId && !ws.data?.shareMember && !ws.data?.widgetDoorGrant) return;
   ...
   * this is that problem's sweep for the one door no share admits.
   *
   * Over the same set the share sweeps walk, not `conns` alone. It was
   * `conns` while the doc's editing socket was the only one the door
   * admitted; the recorder's socket is not in any doc's `conns`, and it is
   * the one that spends a transcription engine for as long as it stays open.
   */
