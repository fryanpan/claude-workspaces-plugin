...
/**
 * The lexicon these checks run on lives in `@claude-workspaces/core`
 * (`speech-lexicon.ts`) — the board asks the same questions of the same
 * speech, and one vocabulary is the point. Re-exported here so this module's
 * own importers are unchanged by the move.
 */
export {
  contentWords,
  isPureBackchannel,
...
import { BACKCHANNEL_OPENER, contentWords, sentencesOf } from '@claude-workspaces/core';
...
export function extractIdeas(turns: readonly NotesTurn[]): SpokenIdea[] {
  ...
  for (const turn of turns) {
    for (const sentence of sentencesOf(turn.text)) {
      ...
      if (BACKCHANNEL_OPENER.test(sentence) && keywords.length < MIN_IDEA_CONTENT_WORDS + 1)
        continue;
      ideas.push({
        turn: turn.turn,
        text: sentence,
