...
describe('mount routes', () => {
  ...
  describe('gates a real socket cannot reach', () => {
    ...
    /**
     * One folder narrower than its project.
     *
     * Two mounts in one project, a caller off the box, and only one of the
     * two marked: the marked folder is refused and the other is served. That
     * is the whole point of the setting — today's project-wide switch would
     * have shut both, and the project holding one outside party's folder
     * would rather shut nothing.
     */
    describe('one mount narrower than its project', () => {
      const twoMounts = (
        store: MountStore,
        ...
        const idOf = (name: string) =>
          files.find((f) => f.relPath.endsWith(name))?.fileId as string;
        return {
          repoKey,
          open: idOf('home.png'),
