...
**Every dispatch prompt tells the builder to end with `report_dispatch`, once.** It carries the PR number, the commit the gates ran on, what each gate did, and a verdict on every done-when line — so you read the build off the board instead of parsing a closing message, and a report that was dropped or sent twice is a board state rather than a guess. The board refuses a report missing any of those and names the part. A second report on the SAME commit is recorded and wakes nobody, so nudging a builder you already heard from costs you nothing; a report on a NEW commit is a new build and does reach you.

**That final message is the agent's ONLY report to you.** Say so in the dispatch prompt: report once, in the final turn, and do not also send the lead a message carrying the same result. You are notified when a dispatch ends and the final message arrives with the notification, so a second copy over `SendMessage` or the hive tells you nothing you are not already about to read — it costs a wake and a turn. In one measured week 97 of 728 builder completions arrived twice this way. If the final message is genuinely dropped — an idle notification with nothing in it — the remedy is the watchdog nudge below, which you initiate; it is not a second copy the agent sends by default.
