 ...
 */
import { secretReadCommand } from '@claude-workspaces/core/secret-name';
 ...
 */
/**
 * The secret shape, and the fields only a TASK item may carry.
 *
 * Not on the comment schema above, and that is deliberate rather than an
 * oversight: the values are handed over on a door addressed by task and item
 * id, so a comment-borne item would be asking for values with nowhere to send
 * them but the ordinary answer path, which records words. The server refuses
 * the shape on that path outright; leaving it out of the schema is how an
 * author learns before the call rather than after it.
 */
    ...
    items: {
      ...
      properties: {
        label: {
          type: 'string',
          description:
            'What the reader is being asked for, in their words. Shown beside the field. 40 characters or fewer.',
        },
        service: {
          type: 'string',
          description:
            'The name the value is stored under, and the only part of it you are ever told. Letters, digits, dot, underscore and dash; 1 to 64 characters; must not start with a dash. Two fields on one item cannot share a name.',
        },
      },
...
const TASK_REVIEW_ITEM_SCHEMA = {
  ...
  description:
    "A review item on this task, with its own headline and its own options. A task can carry several open items at once. Same payload and same refusals as a review item on a comment, plus the 'secret' shape, which only a task item may carry.",
} as const;
...
export const TOOL_LIST: ListToolsResult = {
  tools: [
            ...
            items: {
              ...
              properties: {
                ...
                body: {
                  type: 'string',
                  description:
                    "What the task is for, as a compact user story. Markdown, and next_tasks returns it whole. Do not put the completion criteria here: they go in `doneWhen`, which is the field the board counts and gates on. On a `needs: 'decision'` task the body must carry the question, the stakes and what each option costs, or it is refused.",
                },
                key: {
                  type: 'string',
                  description:
      ...
      inputSchema: {
        ...
        properties: {
          ...
          fields: {
            ...
            items: { type: 'string' },
            description:
              "Project each task to these keys, with `id` always included. Use it for board-wide sweeps, so that heavy fields such as reviews, infoRequests and options do not overflow the result. A name this verb has no key for is REFUSED and named, so a projection that comes back thin is the board and never a dropped entry: `goalTitle` and `ready` are the queue row's keys and belong to next_tasks.",
          },
          includeArchived: {
            type: 'boolean',
            description:
      ...
      inputSchema: {
        ...
        properties: {
          ...
          body: {
            type: 'string',
            description:
              'The FULL new description, replacing what is there. Omit it to leave the body alone. Open with the user story, and keep it readable on a phone. The completion criteria do not go here: send them in `doneWhen`.',
          },
          doneWhen: {
            type: 'array',
            description:
    ...
    },
    {
      ...
      description:
        "Record a person's verbatim answer to a decision task on their behalf, for when they told you in chat or voice. Pass their exact words, never a paraphrase. This answers the task's own decision, and answer_review_item answers one of the items hanging on a task. Neither transitions the task, so close it with task_transition. Neither accepts a 'secret' ask either — the server refuses one, because an answer recorded here is stored, echoed to the feed and read back to you.",
      inputSchema: {
        type: 'object',
        properties: {
          workspaceId: {
    ...
    },
    {
      ...
      description:
        "Say where this project's meetings file and how much of them it keeps. Meetings then land in that folder under the project instead of the server's data dir, and show on the Library page. Retention decides what is WRITTEN, so turning transcripts off keeps the words out of a future meeting and removes nothing already recorded. Machine-scoped: no workspaceId.",
      inputSchema: {
        type: 'object',
        properties: {
          path: {
          ...
          meetingsPath: {
            type: 'string',
            description:
              'The folder from the repo root, e.g. "docs/meetings". No "..", and no dot-directory. It is mounted as it is set.',
          },
          retention: {
            type: 'string',
            enum: ['transcripts-and-audio', 'transcripts', 'none'],
            description:
              "What the project keeps. 'transcripts-and-audio' is the default and today's behaviour; 'transcripts' writes no audio; 'none' writes neither. Left out, the project's current choice stands.",
          },
          gitignore: {
            type: 'boolean',
            description:
              'True writes a .gitignore inside the folder so meetings stay out of git; false removes the one this server wrote. A .gitignore somebody else wrote is never touched.',
          },
        },
    ...
    },
    {
      name: 'set_doc_title',
      description:
        'Rename a doc — the title people and every list know it by. Use it to give a meeting a name instead of the clock it started at. It changes the title only: a bound doc keeps its file path and every comment stays where it is. A blank title, or one over 200 characters, is refused.',
      inputSchema: {
        type: 'object',
        properties: {
          workspaceId: {
