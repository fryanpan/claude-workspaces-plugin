...
export function scheduledRunLine(p: ScheduledRunPayload): string {
  const nth =
    p.attempt !== undefined && p.attempt > 1
      ? ` This is wake ${p.attempt}${p.attempts !== undefined ? ` of ${p.attempts}` : ''}; the board files a review item on the task after the last.`
      : '';
...
export function spawnRequestedLine(p: ScheduledRunPayload): string {
  ...
  return `[task.spawn_requested] ${who} is not attached to board ${
    p.workspaceId ?? '?'
  }, and its scheduled run ${quoted(p.title, p.taskId)} is waiting. Spawn a session for ${who} to run that one task, and spin it down when the task closes. The board asks once per run; if nobody answers it files a review item on the task instead.`;
}
