 ...
 *
 * WHAT A LINE MAY NOT SAY. Nothing about a queue. A filing and a revision are
 * emitted by the STORE; the route judges them afterwards, and a held item "is
 * not on anybody's queue" (`routes/task-review-items.ts`). A reinstatement is
 * the same — that route withholds its own announcement when the reinstated
 * item is still held. So each line names what happened to the ITEM, which is
 * what the frame knows, and anything the frame is merely near goes in the
 * meta: the revision's thread id, the ask's shape.
 *
...
export interface ReviewItemEventPayload {
  ...
  headline?: string;
  /**
   * `review_item.revised` only: the newest asked-back question on the item at
   * the time of the revision.
   *
   * It rides the META and never the line. `latestThreadedQuestion` picks the
   * newest question on the item, not the one this revision responded to, so a
   * line reading "answers thread th-2" would assert something the frame does
   * not know. A reader that wants the exchange has the id.
   */
  threadId?: string;
  /**
...
export function reviewItemTaskLine(event: string, p: ReviewItemEventPayload): string {
  ...
  if (event === 'review_item.revised') {
    return `[review item revised] ${where}${by}`;
  }
