...
describe('review_item.added on a task', () => {
  ...
  it('carries the board, the task and the item in the meta, and never doc_id', async () => {
    ...
    expect(f.meta).toEqual({
      ...
      task_id: 't-ZOz',
      review_item_id: 'ri-4',
      // What KIND of ask it is. In the meta rather than the line: it changes
      // how the reader answers, not whether they look.
      shape: 'decision',
...
describe('review_item.revised on a task — no headline on the wire', () => {
  /**
   * The line says what happened to the ITEM and stops. It used to end
   * "answers thread th-2, back on the queue", and both halves were claims the
   * frame cannot support: the revision is emitted by the store and judged by
   * the route afterwards, so a held item is on nobody's queue, and the thread
   * id is the newest asked-back question on the item rather than the one this
   * revision responded to.
   */
  it('names the item and the task, and carries the thread in the meta', async () => {
    const { frames, messages } = harness();
    ...
    const f = only(frames);
    expect(f.content).toBe('[review item revised] item ri-4 on task t-ZOz by Bryan');
    expect(f.content).not.toContain('queue');
    expect(f.content).not.toContain('th-2');
  ...
  // THE CONTROL on the meta key: a revision that followed no question must
  // not carry a thread_id, or the key says only that the renderer ran.
  it('carries no thread_id when the revision followed no question', async () => {
    const { frames, messages } = harness();
    await messages.emitChannelMessage('review_item.revised', {
      workspaceId: 'w-DRa',
      taskId: 't-ZOz',
    ...
    const f = only(frames);
    expect(f.content).toBe('[review item revised] item ri-4 on task t-ZOz by Bryan');
    expect('thread_id' in f.meta).toBe(false);
  });

  // And the control on `shape`: a revision does not carry one, so the key is
  // conditional rather than always present.
  it('carries no shape on a revision, which has none', async () => {
    const { frames, messages } = harness();
    await messages.emitChannelMessage('review_item.revised', {
      workspaceId: 'w-DRa',
      taskId: 't-ZOz',
    ...
    expect('shape' in only(frames).meta).toBe(false);
  });
});
