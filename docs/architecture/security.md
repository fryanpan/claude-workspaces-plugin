...
Above all of this is a master switch. Off, every outside hostname is refused before any sign-in check runs and every visitor's open connection is dropped, on the share hostname, the collaboration hostname and the owner's own hostname through the tunnel alike. The tailnet widget door is not under it: like the meeting-bot hostname, that name is outside the switch, and what holds it shut is its own short allowlist and the board token each of its routes asks for. Only a program on this machine can throw it. Every flip writes one line to the error log naming who, from which address, when and why.
