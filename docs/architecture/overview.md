...
CSP, and it writes the bridge, the widget and `mockup-live.js` into the
frame's bytes, because an opaque-origin frame's subresource requests carry no
Lax cookie. Three new top-level modules in the widget package are
