...
the directive can never fire on a topic the eval calls fine, and it asks for a
`### ` heading rather than a group whenever no TOPIC names the run — under
nothing, or under a `##` section heading somebody else wrote.
`notes-section-home.ts` joins the same DOMAIN tier and is the other half of
that: it re-addresses a tick's added notes that would land outside the
meeting's own section, so a doc whose notes heading is not its last heading
cannot take a meeting's minutes into somebody's agenda. `notes-unconfirmed.ts`
is the DOMAIN half of settling the guesses a meeting marked "(unconfirmed)" —
it finds them inside the section and names their ids to the cleanup pass,
which counts what is left.
...
| **Domain (pure)** | `task-owner.ts`, `task-fields.ts`, `task-row.ts`, `decision-shape.ts`, `safe-path.ts`, `workspace-path.ts`, `path-params.ts`, `diff-groups.ts`, `pause-ticker.ts`, `keep-moving.ts`, `stall-gate.ts`, `ui-review-gate.ts`, `notes-edit-parse.ts`, `notes-prompt-build.ts`, `notes-invented-links.ts`, `notes-scheme-links.ts`, `notes-research-placeholder.ts`, `ask-detection.ts`, `notes-link-intent.ts`, `notes-idea-coverage.ts`, `notes-edit-guard.ts`, `notes-edit-bullets.ts`, `notes-edit-correction.ts`, `notes-section-fit.ts`, `notes-section-home.ts`, `notes-unconfirmed.ts`, `notes-method.ts` (core), `model-quota.ts`, `notes-notice.ts`, `notes-edit-address.ts`, `dispatch-request-event.ts`, `agent-listening.ts`, `claude-key-source.ts` | Functions over values: no clock, filesystem or socket unless passed in, so a rule is testable without a server. |
